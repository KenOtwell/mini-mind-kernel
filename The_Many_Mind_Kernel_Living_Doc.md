...
Build a tight, packageable Python/FastAPI dual-service architecture (Falcon + Progeny) that fully replaces HerikaServer as the backend for Skyrim VR's CHIM mod. Falcon (Gaming PC) accepts SKSE plugin events via HTTP, computes emotional deltas, embeds text, writes RAW events to Qdrant, retrieves relevant memories, and forwards deltas + keys to Progeny. Progeny (Beelink 395AI) accumulates state across turns, builds context bundles, drives LLM interaction, parses structured responses, and writes MOD/MAX memory tiers directly to Qdrant over LAN. Emotional harmonics drive memory storage, retrieval, and agent behavior via forward-hold credit assignment — no backpropagation.
...
**Qdrant Instance**: `C:\Tools\qdrant\qdrant.exe` — ports 6333 (REST) / 6334 (gRPC)

**Existing Collections** (12 total):
...
The SKSE plugin (AIAgent.dll) communicates via simple HTTP POST. Our FastAPI endpoint must be 100% wire-compatible. **Source of truth**: `abeiro/HerikaServer` branch `aiagent`, files `processor/comm.php` and `main.php` — reverse-engineered March 2026 from downloaded source. Reference copies at `docs/comm_ref.php` and `docs/processor_comm_ref.php`.
...
**Response polling** (Falcon handles locally, never forwarded to Progeny):
...
* `chatnf` — Chat with NPC not found.
* `just_say` — Direct output passthrough. Data = text to output. Returns the text immediately.

**Session lifecycle**:
* `init` — Game load/reload. Data = DLL version string. Triggers cleanup of all DB tables with `gamets >= incoming_gamets` (rollback to save state). Creates "Dragon Break" snapshots before pruning if large gamets rollback detected. Restores NPC state. Clears relationship queues. Loads SNQE quests. Optionally triggers Narrator welcome message.
* `wipe` — Full database reset. Clears all tables unconditionally.
* `playerdied` — Player death. Rolls back all tables to the gamets of the last `infosave` event. Same Dragon Break snapshot logic as `init`.
* `goodnight` — Night cycle event. Triggers auto-diary for nearby NPCs.
* `waitstart` — Player begins waiting/sleeping. Stores gamets. Triggers auto-diary.
* `waitstop` — Player finishes waiting/sleeping. Computes elapsed hours from `waitstart` gamets, logs time-forward event.
...
* `util_location_name`/`named_cell` etc. → split on `/` → typed location/cell structs
* All others → raw string data preserved in a generic `TypedEvent(event_type, local_ts, game_ts, data)` wrapper
...
* Falcon: receives SKSE events, structurally parses via `wire_protocol.py` + typed data parsers, accumulates in time-ordered buffer
* Falcon: on tick (~1-3 seconds), snapshots buffer, ships typed event package to Progeny, clears buffer
* Falcon: handles `request` locally — dequeues from response queue, returns to SKSE. Never involves Progeny for polling.
...
* Parse inbound: `type|localts|gamets|data` → `ParsedEvent` with typed fields
* Split on first 3 pipes (data field may contain pipes)
* Format outbound: response bundles → `NPCName|DialogueType|Text\r\n` or `NPCName|command|ActionName@Params\r\n`
...
* Accumulates typed events in a time-ordered buffer between ticks
* On tick (~1-3 seconds): snapshot buffer, wrap as typed event package, POST to Progeny via `progeny_protocol.py`, clear buffer
* Tick interval is configurable, independent of SKSE POLINT
* Thread-safe: SKSE events arrive on HTTP handler threads, tick runs on a background timer
...
* `POST /comm.php` (or configurable path) — SKSE compatibility endpoint: accepts `type|localts|gamets|data`, parses, accumulates
* `request` events — handled locally: dequeue from response queue, format as CHIM wire, return to SKSE
* All other events — structurally parsed, added to tick accumulator buffer
* `GET /health` — Progeny connectivity status, queue depth, tick stats
